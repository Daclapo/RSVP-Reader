...
const FileDropzone = ({ onFileContent }: FileDropzoneProps) => {
  ...
  return (
    <div
      {...getRootProps()}
      className={`flex flex-col items-center justify-center h-64 border-2 border-dashed rounded-md p-4 transition-colors ${
        isDragActive ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/50' : 'border-gray-300 dark:border-gray-600'
      }`}
    >
      <input {...getInputProps()} />
      {isLoading ? (
        <p className="text-gray-500">Reading file...</p>
      ) : isDragActive ? (
